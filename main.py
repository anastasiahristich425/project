...
############################################################# Кнопки
        ...
        btn_search = tk.Button(toolbar, bg = '#d7d7d7', bd = 1, 
                            image = self.search_img, command = self.open_sreach)
...
############################################################# Создание таблицы          
        ...
        self.tree = ttk.Treeview(self, columns = ('ID', 'name', 'phone', 'email'), 
                                 height = 45, show = 'headings') # Убирается нулевой столбец
...
class Child(tk.Toplevel):
    ...
    def init_child(self):
        ...
        self.geometry('400x220') # Размер окна
        self.resizable(False, False) # Нельзя менять размер окна
        self.grab_set() 
        self.focus_set()
#############################################################
...
class Update(Child):
    ...
    def default_data(self):
        ...
        if not selected_item:
            messagebox.showerror('Ошибка', 'Закройте окно редактирования и выберите запись для редактирования.')
            return
